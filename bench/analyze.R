...
data$Method <- with(data, reorder(Method, Mean))
...
plotMap <- ggplot(dataMap, aes(x=Method, y=Mean, ymin=MeanLB, ymax=MeanUB, color=Name)) +
  scale_y_log10(breaks=10^(0:-8)) +
  geom_errorbar(width=0.5) +
  geom_point()
ggsave(file="benchTMap.png", plot=plotMap, dpi=100, width=14, height=8)

plotSet <- ggplot(dataSet, aes(x=Method, y=Mean, ymin=MeanLB, ymax=MeanUB, color=Name)) +
  scale_y_log10(breaks=10^(0:-8)) +
  geom_errorbar(width=0.5) +
  geom_point()
ggsave(file="benchTSet.png", plot=plotSet, dpi=100, width=14, height=8)

plotSetURI <- ggplot(dataSetURI, aes(x=Method, y=Mean, ymin=MeanLB, ymax=MeanUB, color=Name)) +
  scale_y_log10(breaks=10^(0:-8)) +
  geom_errorbar(width=0.5) +
  geom_point()
